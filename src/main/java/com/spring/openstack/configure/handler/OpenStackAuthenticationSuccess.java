...
public class OpenStackAuthenticationSuccess implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        request.getSession().setAttribute("scopedToken", userInformation.getScopedToken().getId());
        request.getSession().setAttribute("scopedToken", userInformation.getScopedToken());
        request.getSession().setAttribute("tokenExpire", new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSXXX").format(userInformation.getUnscopedToken()));
